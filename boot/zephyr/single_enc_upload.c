...
/*数组先转为对应的16进制字符串*/
static void bytes_array_to_str(uint8_t *input_data, char *output,
                               uint16_t lenght)
{
    uint16_t i = 0, j = 0;
    uint8_t temp;
...
static int calculate_hash(const char *hash_name_string, uint8_t *input,
...
static int calculate_hash_flash_data(const struct flash_area *partition,
                                     size_t length, size_t offset, uint8_t *digest)
{
    int ret;
    int i;
    uint32_t read_times;
    ...
    {
        ret = flash_area_read(partition, i * sizeof(read_buffer) + offset, read_buffer,
                              sizeof(read_buffer));
        if (ret)
        {
...
static bool read_file_header_infor(Rbl_Header_t *header,
...
static bool get_app_file_key_iv(uint8_t *KEY, uint8_t *IV,
                                const Rbl_Header_t *header)
{
    uint8_t i;
    ...
    calculate_hash("SHA256", (uint8_t *)str_buffer, sizeof(str_buffer) - 1,
                   file_aes_key);
    calculate_hash("MD5", (uint8_t *)str_buffer, sizeof(str_buffer) - 1,
                   file_aes_iv);
    for (i = 0; i < 50; i++)
    {
...
static int aes256_cbc_decrypt_mcu_file(const struct flash_area
                                       *storage_partition, const struct flash_area *app_partition, size_t length,
                                       size_t offset, uint8_t *key, uint8_t *iv)
{
    int i;
    int ret;
    uint32_t read_times;
    ...
    for (i = 0; i < read_times; i++)
    {
        ret = flash_area_read(storage_partition, i * sizeof(read_buffer) + offset,
        ...
        if ((ret = mbedtls_aes_crypt_cbc(&aes_ctx, MBEDTLS_AES_DECRYPT,
                                         sizeof(read_buffer), iv_copy, read_buffer, read_buffer)) != 0)
        {
            BOOT_LOG_ERR("Failed to decrypt: -0x%04x\n", -ret);
            break;
        }
        ret = flash_area_write(app_partition, i * sizeof(read_buffer), read_buffer,
                               sizeof(read_buffer));
        if (ret)
        {
...
#ifdef CONFIG_MCUBOOT_USE_HMI_WITH_ALC16
static int aes256_cbc_decrypt_tft_file(const struct flash_area
                                       *storage_partition, const Rbl_Header_t *header, uint8_t *key, uint8_t *iv)
{
    int ret;
    int tft_addr;
    uint16_t i;
    ...
    /*tft进入下载模式*/
    tft_download_mode = go_to_download_mode(header->Tft_Infor.raw_size, tft_addr,
                                            true);
    k_msleep(1000);
    // 复制IV，因为mbedTLS在解密时会修改IV
    memcpy(iv_copy, iv, sizeof(iv_copy));
    ...
    for (i = 0; i < read_times; i++)
    {
        ...
        send_lenght = 0;
        io_led_set(0);
        ret = flash_area_read(storage_partition,
                              i * sizeof(read_buffer) + header->Mcu_Infor.pkg_size + sizeof(Rbl_Header_t),
                              read_buffer, sizeof(read_buffer));
        if (ret)
        {
            BOOT_LOG_ERR("Failed to flash_area_read: 0x%04x", -ret);
        ...
        memcpy(tft_send_buffer + (i % 16) * sizeof(read_buffer), read_buffer,
               sizeof(read_buffer));
        /*发送数据到hmi串口*/
...
int release_image_to_slot(uint8_t app_slot, uint8_t storage_slot,
                          uint32_t enc_image_lenght)
{
    int rc = 0;
    /*打包的头信息*/
    ...
    BOOT_LOG_INF("release_image_to_slot app_slot %d ,enc_image_slot %d size %d",
                 app_slot, storage_slot, enc_image_lenght);
    ...
    if (calculate_hash_flash_data(storage_partition, fw_info.Mcu_Infor.pkg_size,
                                  sizeof(Rbl_Header_t), fw_info.Mcu_Infor.aes_data_sha256))
    {
        rc = -5;
    ...
    if (calculate_hash_flash_data(storage_partition, fw_info.Tft_Infor.pkg_size,
                                  sizeof(Rbl_Header_t) + fw_info.Mcu_Infor.pkg_size,
                                  fw_info.Tft_Infor.aes_data_sha256))
    {
        rc = -6;
    ...
    if (calculate_hash_flash_data(app_partition, fw_info.Mcu_Infor.raw_size, 0,
                                  fw_info.Mcu_Infor.raw_data_sha256))
    {
        rc = -8;
    ...
    {
        rc = -11;
        BOOT_LOG_ERR("can not open the fpag partition error code %d", rc);
        goto out;
    }
    BOOT_LOG_INF("Start erase fpga app partition");
    /*earse fpga app partition*/
    rc = flash_area_erase(fpag_partition, 0, flash_area_get_size(fpag_partition));
    if (rc)
    {
        rc = -12;
        BOOT_LOG_ERR("can not earse the fpag partition error code %d", rc);
        goto out;
    }
